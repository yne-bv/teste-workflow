...
- Create Angular project with ng: `ng new MyWebApp.Client --defaults --skip-install --skip-git --no-standalone `.
...
- Create project file (`MyWebApp.Client.esproj`).
- Create `launch.json` to enable debugging.
- Create `nuget.config` to specify location of the JavaScript Project System SDK (which is used in the first line in `MyWebApp.Client.esproj`).
